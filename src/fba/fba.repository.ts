...
export class FbaRepository {
  ...
  async getFixedClosingFeeData() {
    const data = await readFile(join(process.cwd(),
  ...
  async getProductSizeTierData() {
    const data = await readFile(join(process.cwd(),'./src/fba/data/fulfillmentFees/sizeTier.json'), 'utf8');
    const dataObj = JSON.parse(data);
    return dataObj;
  }
  async getFulfillmentFeeData() {
    const data1 = await readFile(join(
      process.cwd(),'./src/fba/data/fulfillmentFees/fulfillmentFee.json'),
      'utf8',
    );
    const data2 = await readFile(join(process.cwd(),'./src/fba/data/fulfillmentFees/addOn.json'), 'utf8');
  ...
  async getAddOnData() {
    const data = await readFile(join(process.cwd(),'./src/fba/data/fulfillmentFees/addOn.json'), 'utf8');
    const dataObj = JSON.parse(data);
    return dataObj;
  }
  ...
  async getApparelsDetails() {
    const data = await readFile(join(process.cwd(),'./src/fba/data/fulfillmentFees/apparel.json'), 'utf8');
    const dataObj = JSON.parse(data);
    return dataObj;
  }
